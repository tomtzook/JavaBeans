...
public class ObservableFactory {
    ...
    public <T> ObservableProperty<T> newProperty(T initialValue) {
        return new SimpleObservableProperty<T>(new ObservableEventController.Impl<>(mEventController), initialValue);
    }

    public <T> ObservableProperty<T> newConcurrentProperty(T initialValue) {
        return new AtomicObservableProperty<>(new ObservableEventController.Impl<>(mEventController), initialValue);
    }

    public ObservableBooleanProperty newBooleanProperty(boolean initialValue) {
        return new SimpleObservableBooleanProperty(new ObservableEventController.Impl<>(mEventController), initialValue);
    }

    public ObservableBooleanProperty newConcurrentBooleanProperty(boolean initialValue) {
        return new AtomicObservableBooleanProperty(new ObservableEventController.Impl<>(mEventController), initialValue);
    }

    public ObservableIntProperty newIntProperty(int initialValue) {
        return new SimpleObservableIntProperty(new ObservableEventController.Impl<>(mEventController), initialValue);
    }

    public ObservableIntProperty newConcurrentIntProperty(int initialValue) {
        return new AtomicObservableIntProperty(new ObservableEventController.Impl<>(mEventController), initialValue);
    }

    public ObservableDoubleProperty newDoubleProperty(double initialValue) {
        return new SimpleObservableDoubleProperty(new ObservableEventController.Impl<>(mEventController), initialValue);
    }

    public ObservableDoubleProperty newConcurrentDoubleProperty(double initialValue) {
        return new AtomicObservableDoubleProperty(new ObservableEventController.Impl<>(mEventController), initialValue);
    }

    public ObservableLongProperty newLongProperty(long initialValue) {
        return new SimpleObservableLongProperty(new ObservableEventController.Impl<>(mEventController), initialValue);
    }

    public ObservableLongProperty newConcurrentLongProperty(long initialValue) {
        return new AtomicObservableLongProperty(new ObservableEventController.Impl<>(mEventController), initialValue);
    }
}
